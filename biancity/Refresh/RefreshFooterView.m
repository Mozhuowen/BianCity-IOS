...
{
    // 内容的高度
    CGFloat contentHeight = self.scrollView.refresh_contentSizeHeight;
    // 表格的高度
    CGFloat scrollHeight = self.scrollView.refresh_height - self.scrollViewOriginalInset.top - self.scrollViewOriginalInset.bottom;
    // 设置位置和尺寸
    self.refresh_y = MAX(contentHeight, scrollHeight);
}
...
- (void)setState:(RefreshState)state
{
    ...
    switch (state)
    {
        case RefreshStateNormal:
        {
            ...
            if (RefreshStateRefreshing == oldState) {
                ...
                [UIView animateWithDuration:RefreshSlowAnimationDuration animations:^{
                    self.scrollView.refresh_contentInsetBottom = self.scrollViewOriginalInset.bottom;
                }];
            } else {
                // 执行动画
            ...
            if (RefreshStateRefreshing == oldState && deltaH > 0 && currentCount != self.lastRefreshCount) {
                self.scrollView.refresh_contentOffsetY = self.scrollView.refresh_contentOffsetY;
            }
            break;
        }
        ...
        case RefreshStateRefreshing:
        {
            ...
            [UIView animateWithDuration:RefreshFastAnimationDuration animations:^{
                CGFloat bottom = self.refresh_height + self.scrollViewOriginalInset.bottom;
                CGFloat deltaH = [self heightForContentBreakView];
                if (deltaH < 0) { // 如果内容高度小于view的高度
                    bottom -= deltaH;
                }
                self.scrollView.refresh_contentInsetBottom = bottom;
            }];
            break;
        }
