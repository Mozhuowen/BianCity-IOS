...
-(void)loadInfo:(int)check{
    if(check==0){
        [[[basicRequest sharedBaseic] rejectid] removeAllObjects];
    }
    NSDictionary *parameters = [[basicRequest sharedBaseic] paraters];
    //NSLog(@"%@",parameters);
    NSString *url =[NSString stringWithString:[[basicRequest sharedBaseic] gethoturl]];
    ...
    [manager POST:url parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if(check==0){
            ...
            for(int i=0;i<[self.hotTown.towns count];i++){
                ...
                [[[basicRequest sharedBaseic] rejectid] addObject:rjid];
            }
        }else {
